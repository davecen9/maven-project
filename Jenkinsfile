...
stages{
        ...
        stage ('Deployments'){
            parallel{
                stage ('Deploy to Staging'){
                    steps {
                        bat 'winscp -i C:/Users/cigar621/.ssh/admin.pem **/target/*.war ec2-user@${params.tomcat_dev}:/home/ec2-user/apache-tomcat-9.0.37/webapps'
                    }
                }
 
                stage ("Deploy to Production"){
                    steps {
                        bat 'winscp -i C:/Users/cigar621/.ssh/admin.pem **/target/*.war ec2-user@${params.tomcat_prod}:/home/ec2-user/apache-tomcat-9.0.37/webapps'
                    }
                }
            }
        }
